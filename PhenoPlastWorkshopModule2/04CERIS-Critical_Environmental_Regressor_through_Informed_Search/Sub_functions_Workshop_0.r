...
Compile_PTT_PTR_local_GHCN <-  function(exp_dir, env_meta_info, exp_s_year, exp_e_year, searching_daps,ptt_ptr_file, t_base, t_max1, t_max2, Top_dir) {
# env_meta_info <- env_meta_info_0;
 ...
 local_ghcn_target_st_file <- paste(Top_dir, '0ghcn/ghcn_stations_lite.txt', sep = '');
...
# }
 ...
 for (e_y in 1:(exp_e_year - exp_s_year + 1)) {
   gz_ghcn_file <- paste(ghcn_year_dir, exp_s_year + e_y - 1, '.csv.gz', sep = '');
 ...
 for (e_i in 1:nrow(env_meta_info)) { ## 
  ...
  t_df <- data.frame(env_code = env_codes, date = DAPs, TMAX = DL_TM$TMAX, TMIN = DL_TM$TMIN, DL = DL_TM$DL, GDD = GDDs, DTR = DTRs, PTT = PTTs, PTR = PTRs, PTD1 = PTD1s, PTD2 = PTD2s, PTS = PTSs)
...
#   }
 ...
 write.table(line_by_env_df, file = paste(exp_trait_dir, 'LbE_table.txt', sep = ''), sep = "\t", row.names = F, quote = F);
...
#  trait_mean_envPara <- merge(env_mean_trait, env_facts_matrix, stringsAsFactors = F);
  ...
  png(png_file,width= nParas * 1,height= 2 * 2,pointsize= 12, unit = "in", res = 600)

  layout(matrix(c(1:nParas), ncol = nParas / 2, byrow = T));
...
#   envPara <- mean(envParas$PTR[maxR_win]);
 ...
 out_file <- paste(exp_trait_dir, 'Intcp_Slope.txt', sep = '');
